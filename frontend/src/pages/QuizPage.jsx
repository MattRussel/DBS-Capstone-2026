...
const QuizPage = () => {
  ...
  // Di dalam QuizPage.jsx, cari handleStartQuiz:
const handleStartQuiz = async (topicKey) => {
  ...
  try {
    // 🟢 Pastikan rutenya tepat ke API Quiz, bukan Chatbot
    const response = await fetch('http://localhost:5000/api/quiz/start', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        topik: infoMateriKuis[topicKey].tag // Hanya kirim 'topik'
      })
    });
    ...
    // 🟢 Backend kita mengembalikan { success: true, data: [...] }
