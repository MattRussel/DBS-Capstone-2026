...
import axios from 'axios'; // 🟢 Menggunakan Axios agar auto-catch error HTTP 404/500

const ParentPage = () => {
  // --- STATE MANAGEMENT DARI DATABASE ---
  ...
  useEffect(() => {
    const fetchParentDashboardData = async () => {
      ...
      try {
        ...
        // 🟢 1. Ambil Data Kehadiran (daily_checkins) via Axios
        try {
          const checkInRes = await axios.get(`${BACKEND_API_URL}/api/checkin/history/${studentId}`);
          if (Array.isArray(checkInRes.data)) {
            setCheckInLogs(checkInRes.data);
          }
        } catch (e) {
          console.warn("⏳ Jalur /api/checkin/history belum siap atau data kosong di Supabase.");
        }

        // 🟢 2. Ambil Data Hasil Kuis (quiz_scores) via Axios
        try {
          ...
          console.warn("⏳ Jalur /api/quiz/results belum siap atau data kosong di Supabase.");
        }

        // 🟢 3. Ambil Data Chatbot untuk Sensor Kata Kotor (chatbot_history) via Axios
        try {
          ...
          console.warn("⏳ Jalur /api/chatbot/history belum siap atau data kosong di Supabase.");
        }

      } catch (err) {
        console.error("❌ Gagal memuat data dari database Supabase:", err.message);
        setErrorMsg('Gagal terhubung ke API Gateway Node.js. Silakan pastikan server Express kelompokmu sudah dijalankan.');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-[#F5F0E8] font-['Nunito'] space-y-10 animate-fadeIn h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className={`border p-6 rounded-[30px] shadow-sm flex items-center gap-4 transition-all ${totalStrikes > 0 ? 'bg-[#FDE8DC] border-red-300 animate-pulse' : 'bg-white border-[#D6CFC4]'}`}>
          ...
          <div>
            ...
            <p className={`text-2xl font-black mt-0.5 ${totalStrikes > 0 ? 'text-red-600' : 'text-[#2C1A0E]'}`}>{totalStrikes} Pelanggaran</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-[#D6CFC4] p-6 rounded-[35px] shadow-sm space-y-4 w-full">
        <div className="border-b border-[#EAE4D9] pb-3">
          <h3 className="font-black text-lg text-[#2C1A0E] flex items-center gap-2"><span>📅</span> Catatan Log Kehadiran Jurnal Harian (daily_checkins)</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse bg-[#FAF7F2]/40">
            <thead>
              <tr className="bg-[#FAF7F2] border-b border-[#D6CFC4] text-xs font-black text-[#6B5C4E] uppercase tracking-wider">
                ...
                <th className="p-4">Kondisi Perasaan Anak (Mood)</th>
              ...
              {checkInLogs.length === 0 ? (
                <tr>
                  <td colSpan="4" className="p-8 text-center text-xs font-bold text-[#6B5C4E]">
                    Belum ada riwayat absensi harian dari anak Anda. Mari ingatkan anak untuk melakukan Daily Check-In setiap hari! 🌟
                  </td>
                </tr>
              ) : (
                checkInLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-white transition-colors duration-100">
                    <td className="p-4">
                      {log.tanggal_checkin ? new Date(log.tanggal_checkin).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'}
                    </td>
                    <td className="p-4">
                      <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-black border border-green-200">Hadir ✨</span>
                    </td>
                    <td className="p-4">{log.mood || '😀 Bahagia'}</td>
                    <td className="p-4 text-right text-[#7A8C5C] font-black">{log.streak_count || 1} Hari 🔥</td>
                  </tr>
                ))
              )}
            </tbody>
