...
const AuthModal = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  const [showParentPassword, setShowParentPassword] = useState(false); // 🌟 State baru untuk intip sandi orang tua
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    role: 'anak', 
    parent_password: '', // 🟢 Perubahan: parent_id diganti menjadi parent_password
    agreed: false,
  });
  ...
  useEffect(() => {
    ...
    setFormData({ 
      ...
      password: '', 
      role: 'anak', 
      parent_password: '', // 🟢 Reset ke string kosong
      agreed: false 
    });
  }, [activeTab]);
  ...
  const handleSubmit = async (e) => {
    ...
    // 🛡️ VALIDASI KATA SANDI ORANG TUA WAJIB (NON-OPTIONAL JIKA DAFTAR SEBAGAI ANAK)
    if (activeTab === 'register' && formData.role === 'anak' && !formData.parent_password.trim()) {
      setErrorMessage('Waduh, Kata Sandi Orang Tua wajib diisi! 🔒\nIni diperlukan agar Ayah/Bunda bisa masuk ke Ruang Pantau nanti.');
      return;
    }
    ...
    try {
      if (activeTab === 'register') {
        ...
        const response = await axios.post('http://localhost:5000/api/user/register', {
          ...
          role: formData.role,
          // 🟢 Dikirim murni sebagai string sesuai dengan modifikasi database varchar kelompokmu
          parent_password: formData.role === 'anak' ? formData.parent_password : '',
        ...
        if (response.data.success) {
          ...
          // 🟢 Simpan password orang tua ke local storage agar Gate di App.jsx bisa mencocokkannya secara instan
          localStorage.setItem('parent_password', response.data.user.parent_password || '');
      ...
      if (activeTab === 'login' && lowerCaseMessage.includes('belum terdaftar')) {
        ...
        setTimeout(() => { setActiveTab('register'); }, 2500);
      } 
  ...
  return (
    <div className="fixed inset-0 bg-[#2C1A0E]/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto font-['Nunito']" onClick={onClose}>
      ...
      {/* 🟢 PERBAIKAN: Ditambahkan flex-col dan max-h-[90vh] agar modal pas di tengah layar dan tidak terpotong */}
      <div className="bg-[#FAF7F2] rounded-[40px] sm:rounded-[50px] w-full max-w-md my-auto p-6 sm:p-8 lg:p-10 shadow-xl border border-[#D6CFC4] relative flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        
        {/* Header Bar (Tetap diam di posisi atas) */}
        <div className="flex items-center justify-between mb-4 pb-4 border-b-2 border-[#D6CFC4] bg-[#F5F0E8] p-4 rounded-[30px] shrink-0">
        ...
        {/* Tab Navigasi (Tetap diam di posisi atas) */}
        <div className="flex bg-[#F5F0E8] rounded-full p-1.5 mb-4 border-2 border-[#D6CFC4] shrink-0">
          ...
          <AnimatePresence mode="wait">
            {errorMessage && (
              <motion.div 
                ...
                transition={{ type: "spring", stiffness: 500, damping: 25, x: { duration: 0.4 } }}
                className="mb-4 p-4 bg-[#FDE8DC] border border-[#C4621D] text-[#2C1A0E] text-xs sm:text-sm rounded-2xl font-bold whitespace-pre-line shadow-md flex items-start gap-3"
              >
                <motion.span animate={{ rotate: [0, -15, 15, 0] }} transition={{ repeat: Infinity, duration: 1.5, repeatDelay: 1 }} className="text-xl leading-none">⚠️</motion.span>
            ...
            {successMessage && (
              <motion.div 
                ...
                className="mb-4 p-4 bg-[#F5F0E8] border border-[#7A8C5C] text-[#2C1A0E] text-xs sm:text-sm rounded-2xl font-bold shadow-md flex items-center gap-3"
              >
                <motion.span animate={{ y: [0, -5, 0] }} transition={{ repeat: Infinity, duration: 1 }} className="text-xl leading-none">🎉</motion.span>
        ...
        {/* 🟢 PERBAIKAN UTAMA: Area isi form diberikan overflow-y-auto mandiri agar bisa di-scroll ketika input memanjang */}
        <div className="flex-1 overflow-y-auto pr-1.5 space-y-4 scrollbar-thin">
          <form onSubmit={handleSubmit} className="space-y-4 pt-1">
                ...
                {formData.role === 'anak' && (
                  <div className="space-y-1 relative animate-fadeIn">
                    <label className="text-xs font-bold text-red-700 flex items-center gap-1">
                      Kata Sandi Orang Tua <span className="text-[10px] bg-red-100 px-2 py-0.5 rounded-md border border-red-200">WAJIB</span>
                    </label>
                    <div className="relative">
                      <input 
                        ...
                        placeholder="Buat sandi khusus orang tua..." 
                        className="w-full px-5 py-3 pr-12 border border-red-300 bg-white text-sm rounded-xl outline-none font-bold text-[#2C1A0E] focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all" 
                        required={formData.role === 'anak'} 
                      />
                      <button 
                        type="button" 
                        onClick={() => setShowParentPassword(!showParentPassword)} 
                        className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#6B5C4E] hover:text-red-700 hover:bg-red-50 transition-colors p-1.5 rounded-full"
                      >
                        {showParentPassword ? (
            ...
            <div className="space-y-1 relative">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#6B5C4E] hover:text-[#C4621D] hover:bg-[#FDE8DC] transition-colors p-1.5 rounded-full">
                  {showPassword ? (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                  ) : (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.822 7.822L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" /></svg>
            ...
            {/* Tombol Submit Utama */}
            <button type="submit" className="w-full bg-[#2C1A0E] text-[#FAF7F2] font-extrabold py-3.5 rounded-full mt-2 hover:bg-[#3B2314] transition-all shadow-md text-sm tracking-wide focus:ring-4 focus:ring-[#7A8C5C]/30 outline-none">
          ...
          <button 
            ...
            className="w-full bg-white border-2 border-[#D6CFC4] text-[#2C1A0E] font-bold py-3 rounded-full shadow-sm hover:border-[#7A8C5C] hover:bg-[#FAF7F2] transition-all text-sm focus:ring-4 focus:ring-[#D6CFC4]/30 outline-none mb-2"
          >
